    ...
    stages {
        ...
        stage('Build dev') {
            ...
            steps {
                ...
                script {
                  ...
                  docker.build("${env.GIT_REPO_NAME}/${env.BRANCH_NAME}:${env.BUILD_ID}", "--network host -f Dockerfile-dev .")
                        }
            }
        }
        ...
        stage('Dev Deploy') {
            ...
            steps {
                ...
                sh "kubectl config use-context arn:aws:eks:us-east-1:705666639440:cluster/kub02 ; kubectl -n $BRANCH_NAME-$GIT_REPO_NAME set image deployment $BRANCH_NAME-$GIT_REPO_NAME=705666639440.dkr.ecr.us-east-1.amazonaws.com/$GIT_REPO_NAME/$BRANCH_NAME:$BUILD_ID --all"
       ...
       stage('Feature Branch Deploy') {
            ...
            steps {
                 ...
                 // Upload the build to ECR
                sh 'aws ecr describe-repositories --repository-names $GIT_REPO_NAME/$BRANCH_NAME || aws --profile stg ecr create-repository --repository-name $GIT_REPO_NAME/$BRANCH_NAME'
                ...
                script {
                  ...
                  // Apply the files to K8s
                  sh 'cd /var/jenkins_home ; rm -rf helm-charts ; GIT_SSH_COMMAND="ssh -o StrictHostKeyChecking=no" git clone git@gitlab.com:betterchange/devops/helm-charts.git'
                  sh 'cd /var/jenkins_home/helm-charts ; ./feature_deploy-stgv2.sh $GIT_REPO_NAME $BRANCH_NAME'

                  // Update K8s deployment with the new ECR image
                sh "kubectl config use-context arn:aws:eks:eu-central-1:840916144018:cluster/joonko-stgv2 ; kubectl -n $GIT_REPO_NAME-$BRANCH_NAME set image deployment $GIT_REPO_NAME-$BRANCH_NAME=705666639440.dkr.ecr.us-east-1.amazonaws.com/$GIT_REPO_NAME/$BRANCH_NAME:$BUILD_ID --all"
       ...
       stage('Stage Deploy') {
            ...
            steps {
                ...
                // Update K8s deployment with the new ECR image
                sh "kubectl config use-context arn:aws:eks:us-east-1:705666639440:cluster/kub02 ; kubectl -n stg-${env.GIT_REPO_NAME} set image deployment stg-${env.GIT_REPO_NAME}=705666639440.dkr.ecr.us-east-1.amazonaws.com/${env.GIT_REPO_NAME}/${env.BRANCH_NAME}:${env.BUILD_ID} --all"
