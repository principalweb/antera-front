...
@Component({
  ...
  styleUrls: ['./accounts-import.component.scss']
})
export class AccountsImportComponent implements OnInit {
    ...
    private api: ApiService
        ...
        this.importAccountsForm = this.createImportAccountsForm();
        ...
        this.confirmDialogRef = this.dialog.open(FuseConfirmDialogComponent, {
            ...
            this.confirmDialogRef.afterClosed().subscribe(result => {
               if ( result )
                {
                  this.loading = true;
                  this.api.downloadAccountsTempalte().subscribe((res: any) => {
                    this.loading = false;
                    const options = {
                      fieldSeparator: ',',
                      quoteStrings: '"',
                  ...
                  new Angular5Csv(data, 'Accounts_template', options);
                  })
                } else {
                  window.location.href='/protected/content/get-accounts-import-template';
                }
                this.confirmDialogRef = null;
            });
    }

    initImportAccountsForm()
    {
        this.importAccountsForm = this.createImportAccountsForm();
        this.currentFile = "No File Chosen";
        this.totalRecords = "";  
    ...
    onFileChange(event,type) {
        if(event.target.files.length > 0) {
            ...
            this.importAccountsForm.get('importAccountsFile').setValue(file);
            this.isImportDone = false;    
            this.totalRecords = "";  
            this.processedRecords = "";  
            this.errorRecords = "";  
    ...
    uploadCSV(type) {
        if (this.importAccountsForm.invalid) {
            this.msg.show('Please complete the form first', 'error');
            return;
        }

        if (!this.importAccountsForm.get('importAccountsFile').value)
        return;

        const data = new FormData();
        data.append('File', this.importAccountsForm.get('importAccountsFile').value);
        this.loading = true;
        this.isImportDone = false;
        this.api.uploadImportFile(data)
        .subscribe((res: any) => {
            this.importAccountsForm.get('importCloudFile').setValue(res.msg);
            const initImportData = new FormData();        
            initImportData.append('importCloudFile', res.msg);
            initImportData.append('module', "Accounts");
