...
export class SiteComponent implements OnInit {
  ...
  constructor(
    ...
    public dialog: MatDialog
        ...
        this.importsForm = this.createImportsForm();
  ...
  ngOnInit() {
    ...
    downloadTemplate() {
        ...
        this.confirmDialogRef.afterClosed().subscribe(result => {
            ...
            {
            this.loading = true;
            this.api.downloadWarehouseTemplate().subscribe((res: any) => {
                this.loading = false;
                const options = {
                fieldSeparator: ',',
                quoteStrings: '"',
            ...
            new Angular5Csv(data, 'Warehouse_template', options);
            })
            } else {
            window.location.href='/protected/content/get-site-import-template';
            }
    ...
    onFileChange(event,type) {
        if(event.target.files.length > 0) {
            ...
            this.importsForm.get('importsFile').setValue(file);
            this.isImportDone = false;    
            this.totalRecords = "";  
            this.processedRecords = "";  
            this.errorRecords = "";  
    ...
    uploadCSV(type) {
        if (this.importsForm.invalid) {
            this.msg.show('Please complete the form first', 'error');
            return;
        }

        if (!this.importsForm.get('importsFile').value)
        return;

        const data = new FormData();
        data.append('File', this.importsForm.get('importsFile').value);
        this.loading = true;
        this.isImportDone = false;
        this.api.uploadImportFile(data)
        .subscribe((res: any) => {
            this.importsForm.get('importCloudFile').setValue(res.msg);
            const initImportData = new FormData();        
            initImportData.append('importCloudFile', res.msg);
            initImportData.append('module', "Site");
