...
@Component({
  selector: 'product-pricing-editor',
  templateUrl: './product-pricing-editor.component.html',
  styleUrls: ['./product-pricing-editor.component.scss']
})
export class ProductPricingEditorComponent implements OnInit {
  ...
  ngOnInit() {
    console.log('receiving product ', this.product);
    this.setupFilters();
  ...
  saveAttributes() {
    if(this.product && this.product.ProductPartArray.ProductPart){
  ...
  generateUsedAttributes() {
    ...
    if (this.product.ProductPartArray){
        if (this.product.ProductPartArray.ProductPart){
