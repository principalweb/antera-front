...
@Injectable()
export class EcommerceOrderService implements Resolve<any>, OnDestroy
{
    private destroy$ = new Subject<boolean>();

    routeParams: any;
    onOrderChanged: BehaviorSubject<any> = new BehaviorSubject({});
    orderDetails: BehaviorSubject<OrderDetails> = new BehaviorSubject<OrderDetails>(null);
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | any
    {
        ...
        if (route.queryParams) {
            ...
            } else if (route.queryParams.displayTab) {
                this.selectedIndex = route.queryParams.displayTab;
    ...
    autoTaxJar() {
            return this.api.post('/content/connector-get-config', { connector: 'TAXJAR', configs: { 0: 'autoTaxCalc' } });
    }
    ...
    createPurchaseOrderNeed() {
        if(this.order.id) {
            return this.api.post('/api/v2/purchase-needs/sales-order-need', {id:this.order.id});
        }
    }

    createPendingInvoice(){
        if (this.order.id){
            return this.api.createPendingInvoice({id: this.order.id});
        }
    }

    processOrderExchangeRate(){
        if (this.order.id){
            return this.api.processOrderExchangeRate({id: this.order.id})
        }
    }
