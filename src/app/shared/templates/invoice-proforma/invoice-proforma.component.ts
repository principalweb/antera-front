...
@Component({
    selector: 'antera-invoice-proforma',
    templateUrl: './invoice-proforma.component.html',
    styleUrls: ['./invoice-proforma.component.scss'],
    animations: fuseAnimations
})
export class AnteraInvoiceProformaComponent {
    @Input() invoice: any;
    ...
    updateInvoiceDate() {
        this.editingInvoiceDate = !this.editingInvoiceDate;
    }

    onDate(event) {
        this.editingInvoiceDate = false;
        this.loading = true;
        this.invoice.proformaDate = moment(event.value).format('YYYY-MM-DD');
        this.documentService.updateProformaDate({ order_id: this.invoice.id, date: this.invoice.proformaDate  })
            .subscribe((res) => {
                this.loading = false;
            }, (err) => {
                this.loading = false;
